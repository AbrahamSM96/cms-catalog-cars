...
export const metadata: Metadata = {
  title: "Contacto",
  description: "Ponte en contacto con nosotros: WhatsApp, teléfono, correo y ubicación.",
};
...
export default async function ContactoPage() {
  ...
  const whatsappDigits = (contact?.whatsapp || FALLBACK_WHATSAPP).replace(/\D/g, "");
  const phone = contact?.phone || FALLBACK_PHONE;
  const telHref = `tel:${phone.replace(/[^\d+]/g, "")}`;
  const email = contact?.email;
  const addr = contact?.address;
  const addressText = addr
    ? [addr.line1, [addr.postalCode, addr.city, addr.state].filter(Boolean).join(", "), addr.country]
        .filter(Boolean)
        .join(", ")
    : null;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="relative overflow-hidden bg-white pt-28 pb-12 sm:pt-32">
        ...
        <div className="relative mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
          <h1 className="text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">Contacto</h1>
          <p className="mx-auto mt-3 max-w-2xl text-lg text-slate-600">
            Estamos para ayudarte. Escríbenos o visítanos y con gusto te atendemos.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <a
              ...
              className="group flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-soft transition-all hover:-translate-y-0.5 hover:border-green-300 hover:shadow-float"
            >
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-slate-500">Respuesta rápida a tus dudas</p>
                <span className="mt-2 inline-block text-sm font-semibold text-green-600">Escríbenos →</span>
              </div>
            </a>
            ...
            <a
              href={telHref}
              className="group flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-soft transition-all hover:-translate-y-0.5 hover:border-red-300 hover:shadow-float"
            >
              ...
              <div>
                ...
                <span className="mt-2 inline-block text-sm font-semibold text-red-600">Llamar →</span>
              </div>
            </a>
            ...
            {email && (
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-0.5 break-all text-sm text-slate-500">{email}</p>
                  <span className="mt-2 inline-block text-sm font-semibold text-red-600">Enviar correo →</span>
                </div>
              </a>
            )}
          ...
          {/* Hours */}
          {hours && (
            <div className="mt-4 flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-soft">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-700">
                <Clock className="h-6 w-6" />
              </span>
              <div>
                <h2 className="font-semibold text-slate-900">Horario</h2>
                <p className="mt-0.5 whitespace-pre-line text-sm text-slate-500">{hours}</p>
              </div>
            </div>
          )}
        </div>
