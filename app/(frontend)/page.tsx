...
import { getFeaturedCars, getHomepage, getDealerships } from "@/lib/payload-client";
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-slate-50 py-20 sm:py-24">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-soft">
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-red-400 opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-red-500" />
            </span>
            <span className="text-sm font-medium text-slate-700">Inventario actualizado</span>
          </div>
          <h2 className="mt-6 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
            Explora todo nuestro inventario
          </h2>
          <p className="mx-auto mt-3 max-w-2xl text-lg text-slate-600">
            Busca por marca, modelo o versión y filtra por año, transmisión y estado para encontrar tu
            próximo auto.
          </p>
          <div className="mt-8">
            <Link
      ...
      {dealerships.length > 0 && (
        <section id="ubicaciones" className="bg-white py-20 sm:py-24">
