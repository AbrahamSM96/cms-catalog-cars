import { BadgeCheck, MapPin, Fuel, Settings2, Gauge, Palette, type LucideIcon } from "lucide-react";
...
export function CarHeader({ car }: CarHeaderProps) {
  ...
  const buildTechSpecs = () => {
    ...
    if (car.transmission) specs.push(car.transmission === "automatic" ? "Ta" : "Tm");
  ...
  if (car.transmission)
    pills.push({ icon: Settings2, label: car.transmission === "automatic" ? "Automática" : "Manual" });
  if (car.mileage)
    pills.push({ icon: Gauge, label: `${new Intl.NumberFormat("en-US").format(car.mileage)} km` });
    ...
    <div className="mb-8">
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <BadgeCheck className="h-5 w-5 text-red-600" aria-hidden="true" />
        </div>
      </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          {techSpecs && <p className="mt-2 text-lg text-slate-500">{techSpecs}</p>}
      ...
      {pills.length > 0 && (
        <div className="mt-5 flex flex-wrap gap-2">
          {pills.map(({ icon: Icon, label }) => (
            <span
              key={label}
              className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 shadow-soft"
            >
              <Icon className="h-4 w-4 text-slate-400" strokeWidth={1.8} aria-hidden="true" />
              {label}
            </span>
          ))}
        </div>
