...
import { Search, Phone, Navigation, MapPin } from "lucide-react";
...
function coordsOf(d: Dealership) {
...
function addressLines(d: Dealership): string[] {
  const a = d.address;
  if (!a) return [];
  const rest = [a.neighborhood, a.postalCode, a.city, a.state, a.country].filter(Boolean).join(", ");
  return [a.line1, rest].filter(Boolean) as string[];
}

export function Locations({ dealerships }: LocationsProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return dealerships.filter((d) => {
      const hay = [d.name, d.address?.line1, d.address?.neighborhood, d.address?.city, d.address?.state, d.address?.postalCode]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  const withCoords = useMemo(() => dealerships.filter((d) => coordsOf(d) !== null), [dealerships]);
  ...
  useEffect(() => {
    ...
    cardRefs.current[String(selectedId)]?.scrollIntoView({ behavior: "smooth", block: "nearest" });
  }, [selectedId]);
  ...
  if (dealerships.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center">
        ...
        <h3 className="mt-4 text-lg font-bold text-slate-900">Aún no hay ubicaciones</h3>
        <p className="mt-1 text-slate-500">Agrega concesionarios en el panel de administración.</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-6 lg:h-[76vh] lg:grid-cols-[minmax(0,440px)_1fr]">
      {/* Left: search + list */}
      <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
        {/* Search */}
        <div className="border-b border-slate-100 p-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full cursor-text rounded-xl border border-slate-200 bg-white py-3 pl-12 pr-4 text-slate-900 transition-colors placeholder:text-slate-400 focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-500/20"
            />
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 divide-y divide-slate-100 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            filtered.map((d) => {
              ...
              const mapsHref =
                d.googleMapsUrl ||
                (c ? `https://www.google.com/maps/search/?api=1&query=${c.lat},${c.lng}` : null);
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-20 w-24 shrink-0 overflow-hidden rounded-xl bg-slate-100">
                    {img ? (
                      <Image src={img} alt={d.name} fill className="object-cover" sizes="96px" unoptimized />
                    ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {status && (
                      <p className="mt-0.5 text-sm">
                        <span className={clsx("font-semibold", status.open ? "text-emerald-600" : "text-red-600")}>
                          {status.label}
                        </span>
                    ...
                    {lines.length > 0 && (
                      <p className="mt-1 text-sm leading-snug text-slate-600">{lines.join(" · ")}</p>
                    )}
      ...
      {/* Right: map */}
      <div className="h-[50vh] overflow-hidden rounded-2xl border border-slate-200 shadow-soft lg:h-auto">
        <LocationsMap
