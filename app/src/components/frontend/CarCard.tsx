...
export function CarCard({ car }: CarCardProps) {
  ...
  return (
    <Link href={`/catalogo/${buildCarSlug(car)}`} className="group block">
      <article className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-float">
        {/* Image */}
        ...
        <div className="p-5">
          <div className="mb-3">
            ...
            <p className="mt-0.5 truncate text-sm font-medium text-slate-500">{car.version}</p>
          </div>
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <div className="text-2xl font-bold text-slate-900">{formattedPrice}</div>
            </div>
            ...
            <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-red-600 transition-all duration-300 group-hover:gap-2.5">
              Ver detalles
              <svg className="h-5 w-5 transition-transform duration-300 group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
